...
/**GepAlgConfiguration**/
	...
	public void testCreate() {
		/**
		 * Create 返回设置了DataSet和GepAlgConfiguration后的GepAlgRun对象
		 * 疑问：
		 * 该函数的内部变量Population
		 * 有设置操作，
		 * 却没有向外部传参
	...
	public void testSelect() {
		 ...
		 * 无法测试
		 */
		/*Population expect = new Population();
		float sumFitness = 100f;
		List<Float> probability = new ArrayList<Float>();
		probability.add(10f/sumFitness);
		probability.add(20f/sumFitness);
		probability.add(10f/sumFitness);
	...
	public void testMutate() {
		population.getGepAlgRun().getGepAlgConfiguration().getOperatorConfiguration().setMutateRate(1f);
	...
	public void testIsTransport() {
		population.getGepAlgRun().getGepAlgConfiguration().getOperatorConfiguration().setIsTransportRate(1f);
		Population changedPopulation = population.clone();
	...
	public void testRisTransport() {
		population.getGepAlgRun().getGepAlgConfiguration().getOperatorConfiguration().setRisTransportRate(1f);
		Population changedPopulation = population.clone();
	...
	public void testGeneTransport() {
		population.getGepAlgRun().getGepAlgConfiguration().getOperatorConfiguration().setGeneTransportRate(1f);
		Population changedPopulation = population.clone();
	...
	public void testOnePointRecombine() {
		population.getGepAlgRun().getGepAlgConfiguration().getOperatorConfiguration().setOnePointRecombineRate(1f);
		Population changedPopulation = population.clone();
	...
	public void testTwoPointRecombine() {
		population.getGepAlgRun().getGepAlgConfiguration().getOperatorConfiguration().setTwoPointRecombineRate(1f);
		Population changedPopulation = population.clone();
	...
	public void testGeneRecombine() {
		population.getGepAlgRun().getGepAlgConfiguration().getOperatorConfiguration().setGeneRecombineRate(1f);
		Population changedPopulation = population.clone();

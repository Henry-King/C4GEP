...
public final class DataTable {
	...
	/**
	 * 保存行的集合 用来保存DataRowCollection的集合
	 */
	private DataRowCollection rows;

	/**
	 * 保存列的集合 用于保存DataColumn的对象
	 */
		...
		for (int i = 0; i < this.rows.size(); i++) {
			...
			for (int j = 0; j < this.columns.size(); j++) {
				...
				if (obj2.toString().equals(obj.toString())) {
					return row.getColumns().get(j).getColumnName()
							.toLowerCase();
				}
			}
		}
	...
	/*
	 * Iterator it=columns.nameMap.entrySet().iterator(); while(it.hasNext()){
	 * Entry entry=(Entry) it.next(); System.out.println("ww"+entry.getKey());
	 * if(entry.getValue().equals(obj)){ return (String) entry.getKey(); }
	 * 
	 * }
	 */
